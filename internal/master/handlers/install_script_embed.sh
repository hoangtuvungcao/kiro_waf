...
download_xdp_object() {
    ...
    # Load XDP program on eth0 using ip command
    if command -v ip &>/dev/null; then
        ip link set dev eth0 xdpgeneric obj "$XDP_FILE" sec xdp 2>/dev/null || \
            log_warn "Không thể load XDP program trên eth0. Sẽ được load khi service khởi động."
    fi

    # Install xdp-sync-blocklist.sh to /usr/local/bin/kiro-xdp-sync
    cat > /usr/local/bin/kiro-xdp-sync << 'XDPSYNC'
#!/bin/bash
...
exit 0
XDPSYNC
    chmod +x /usr/local/bin/kiro-xdp-sync
    log_success "Đã cài đặt XDP sync script tại /usr/local/bin/kiro-xdp-sync"
}
...
[Service]
Type=simple
ExecStartPre=-/sbin/ip link set dev eth0 xdpgeneric obj /usr/lib/kiro/xdp/xdp_filter.o sec xdp
...
client:
  ...
  listen_addr: ":8090"
  xdp_sync_command: /usr/local/bin/kiro-xdp-sync
EOF
