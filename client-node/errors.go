// Package main triển khai xử lý lỗi và trang lỗi có thương hiệu cho Client_WAF.
...
// SuspendedPageHTML là trang HTML thông báo dịch vụ bị tạm ngưng.
// Hiển thị khi license có trạng thái suspended (Requirement 14.10).
// Thiết kế tông tối nhất quán với các trang lỗi khác.
const SuspendedPageHTML = `<!DOCTYPE html>
<html lang="vi">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Kiro - Dịch Vụ Bị Tạm Ngưng</title>
<style>
:root {
  color-scheme: dark;
  --bg: #07100f;
...
<h1>Dịch vụ bị tạm ngưng</h1>
<p class="description">License bảo vệ cho website này đã bị tạm ngưng bởi quản trị viên. Dịch vụ WAF hiện không hoạt động cho đến khi license được kích hoạt lại.</p>

<div class="status-box">
<div class="status-row">
<span class="status-dot"></span>
<span class="status-label">License tạm ngưng (Suspended)</span>
</div>
</div>

<p class="retry-hint">Vui lòng liên hệ quản trị viên hệ thống để kích hoạt lại dịch vụ.</p>
...
// ServeSuspendedPage ghi trang thông báo tạm ngưng vào ResponseWriter.
// Được sử dụng khi license có trạng thái suspended (Requirement 14.10).
func ServeSuspendedPage(w http.ResponseWriter) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	w.Header().Set("Cache-Control", "no-store, no-cache, must-revalidate")
	w.Header().Set("X-Content-Type-Options", "nosniff")
	w.WriteHeader(http.StatusForbidden)
	_, _ = w.Write([]byte(SuspendedPageHTML))
}
