...
//
// Kiro XDP/eBPF edge drop program.
//
// Build:
//   clang -O2 -g -target bpf -D__TARGET_ARCH_x86 -Wall -Werror \
//     -c client-node/xdp_filter.c -o build/client/xdp_filter.o
//
...
/* Per-IP and per-subnet rate limiting state (LRU eviction) */
...
/* Increment a per-CPU statistics counter atomically. */
 ...
 *
 * Processing flow:
 *   1. Parse Ethernet → IPv4 header (non-IPv4 passes through)
...
int kiro_xdp_drop(struct xdp_md *ctx)
{
	...
	/* Non-IPv4 traffic passes through unconditionally */
