...
if [ ! -f "${ROOT}/master-server/main.go" ]; then
  echo "ERROR: project root is invalid (master-server/main.go not found): ${ROOT}" >&2
  exit 1
fi

if [ ! -f "${ROOT}/client-node/client_waf.go" ]; then
  echo "ERROR: project root is invalid (client-node/client_waf.go not found): ${ROOT}" >&2
  exit 1
fi

if [ ! -f "${ROOT}/client-node/xdp_filter.c" ]; then
  echo "ERROR: XDP source not found: ${ROOT}/client-node/xdp_filter.c" >&2
  exit 1
fi
...
cd "${ROOT}"
"${GO_BIN}" build -trimpath -ldflags "-s -w" -o "${BUILD_DIR}/kiro-master" ./master-server
...
if [ "${INSTALL_CLIENT}" = "true" ]; then
  ...
  "${GO_BIN}" build -trimpath -ldflags "-s -w" -o "${BUILD_DIR}/kiro-client-waf" ./client-node
  ...
  if [ -f "${ROOT}/client-node/kiro-client-update.sh" ]; then
    install -m 0755 "${ROOT}/client-node/kiro-client-update.sh" /usr/local/bin/kiro-client-update
    log_ok "Installed /usr/local/bin/kiro-client-update"
  fi
fi

###############################################################################
# Step 8: Build XDP object: clang -Wall -Werror -O2 -target bpf
###############################################################################
log "Building XDP object file"
bash "${ROOT}/scripts/build-xdp.sh" "${ROOT}/client-node/xdp_filter.c" "${BUILD_DIR}/xdp_filter.o"
...
# kiro-master.service
install -m 0644 "${ROOT}/master-server/systemd/kiro-master.service" /etc/systemd/system/kiro-master.service
log_ok "Installed kiro-master.service"

# kiro-client-waf.service
if [ "${INSTALL_CLIENT}" = "true" ]; then
  install -m 0644 "${ROOT}/client-node/systemd/kiro-client-waf.service" /etc/systemd/system/kiro-client-waf.service
...
sed \
  ...
  "${ROOT}/master-server/nginx/firewall.conf" >/etc/nginx/sites-available/firewall.conf
